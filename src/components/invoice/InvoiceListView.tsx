    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
        ...
        <button
          ...
          disabled={isReadOnly}
          title={isReadOnly ? 'Action disabled in read-only mode' : 'Create New Bill'}
                      ...
                      >
                        <div className="flex items-center justify-end gap-1.5">
                          {inv.invoiceType === InvoiceType.QUOTATION && (
                            inv.isConverted ? (
                              ...
                              <button
                                ...
                                disabled={isReadOnly}
                                title={isReadOnly ? 'Action disabled in read-only mode' : 'Convert Quotation to Official Tax Invoice (Posts to Ledger)'}
                                className={`px-2.5 py-1 text-white font-bold rounded-lg text-xs flex items-center gap-1 shadow-xs transition-colors ${
                                  isReadOnly
                                    ? 'bg-slate-400 opacity-60 cursor-not-allowed'
                          ...
                          <button
                            ...
                            disabled={isReadOnly}
                            title={isReadOnly ? 'Action disabled in read-only mode' : 'Edit Bill'}
                          ...
                          <button
                            ...
                            disabled={isReadOnly}
                            title={isReadOnly ? 'Action disabled in read-only mode' : 'Duplicate Bill'}
                            className={`p-1.5 rounded-lg transition-colors ${
                              isReadOnly
                                ? 'text-slate-300 opacity-50 cursor-not-allowed'
                                : 'text-slate-400 hover:text-slate-900 hover:bg-slate-100 cursor-pointer'
                          ...
                          <button
                            ...
                            disabled={isReadOnly}
                            title={isReadOnly ? 'Action disabled in read-only mode' : 'Delete Bill'}
                            className={`p-1.5 rounded-lg transition-colors ${
                              isReadOnly
                                ? 'text-slate-300 opacity-50 cursor-not-allowed'
                      ...
                      {inv.invoiceType === InvoiceType.QUOTATION && (
                        inv.isConverted ? (
                          ...
                          <button
                            onClick={() => onConvertQuotation(inv)}
                            disabled={isReadOnly}
                            title={isReadOnly ? 'Action disabled in read-only mode' : 'Convert'}
                            className={`px-2 py-1 text-white font-bold rounded-lg text-xs flex items-center gap-1 shadow-xs ${
                              isReadOnly
                                ? 'bg-slate-400 opacity-60 cursor-not-allowed'
