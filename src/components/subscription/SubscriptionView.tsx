...
export const SubscriptionView: React.FC<SubscriptionViewProps> = ({
  ...
  const handleSimulateTrialState = (state: 'TRIAL_ACTIVE' | 'TRIAL_EXPIRED' | 'ACTIVE_PAID') => {
    ...
    if (updated.activePlan) {
      onUpgradeSuccess(updated.activePlan);
    } else {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="px-4 py-2 bg-slate-100/90 rounded-xl border border-slate-200 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-600">
        <span className="font-semibold text-[11px] text-slate-700 flex items-center gap-1.5">
          <Clock className="w-3.5 h-3.5 text-slate-500" />
          <span>Trial Status Tester:</span>
        </span>
        <div className="flex items-center gap-1.5 flex-wrap">
          <button
            type="button"
            onClick={() => handleSimulateTrialState('TRIAL_EXPIRED')}
            className={`px-2.5 py-1 rounded-lg text-[11px] font-bold cursor-pointer transition-colors border ${
              isTrialExpired
                ? 'bg-rose-600 text-white border-rose-700 shadow-xs'
                : 'bg-white hover:bg-rose-50 text-rose-700 border-rose-200'
            }`}
            title="Simulate trial expiration to test read-only mode and blocked invoicing"
          >
            Simulate Trial Expired
          </button>
          <button
            type="button"
            onClick={() => handleSimulateTrialState('TRIAL_ACTIVE')}
            className="px-2.5 py-1 rounded-lg text-[11px] font-semibold bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 cursor-pointer transition-colors"
            title="Reset to 14 days active trial"
          >
            Reset Trial (14 Days)
          </button>
          <button
            type="button"
            onClick={() => handleSimulateTrialState('ACTIVE_PAID')}
            className="px-2.5 py-1 rounded-lg text-[11px] font-bold bg-white hover:bg-emerald-50 text-emerald-700 border border-emerald-200 cursor-pointer transition-colors"
            title="Simulate full active 1-year paid plan"
          >
            Activate Paid Plan
          </button>
        </div>
