...
export const PaymentGatewaysView: React.FC = () => {
  ...
  React.useEffect(() => {
    let isMounted = true;
    setIsLoading(true);
    SaaSAdminDB.getPaymentGatewaysConfigAsync()
      .then((cfg) => {
        if (isMounted && cfg) {
          setManagerConfig(cfg);
        }
      })
      .catch((err) => console.error('Failed to load payment gateways config:', err))
      .finally(() => {
  ...
  const handleSetActive = async (provider: PaymentGatewayProvider) => {
    await SaaSAdminDB.setActivePaymentGateway(provider);
    setManagerConfig((prev) => ({ ...prev, activeProvider: provider }));
    showToast(`Active payment gateway switched to ${managerConfig.gateways[provider]?.name || provider}!`);
  };
  ...
  const handleSaveGateway = async (provider: PaymentGatewayProvider) => {
    await SaaSAdminDB.updateGatewayConfig(provider, managerConfig.gateways[provider]);
    showToast(`Saved settings for ${managerConfig.gateways[provider]?.name || provider}!`);
  };

  const handleTestConnection = async (provider: PaymentGatewayProvider) => {
    ...
    if (provider === 'payu') {
      try {
        ...
        const res = await fetch('/api/admin/payments/payu/test-connection', {
          ...
          body: JSON.stringify({
            merchantKey: gw.payuMerchantKey,
            merchantSalt: gw.payuMerchantSalt,
        ...
        setTestResult({
          ...
          message:
            data.message ||
            (data.success
              ? `PayU Gateway verified (${gw.isTestMode ? 'Sandbox' : 'Production'}). Post-Checkout SHA-512 Hash Verified.`
              : 'PayU connection test failed.'),
        });
      } catch (err: any) {
        setTestResult({
          provider,
          success: false,
          message: `PayU Connection error: ${err?.message || 'Network error'}`,
        });
      } finally {
