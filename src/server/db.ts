...
export async function ensureTables(db: D1Database): Promise<void> {
  try {
    ...
    // Migration: ensure subscription_transactions has coupon_code
    try {
      const subInfo = await db.prepare('PRAGMA table_info(subscription_transactions)').all<{ name: string }>();
    ...
    // Migration: upgrade platform_settings schema if old section-based table exists
    try {
      const psInfo = await db.prepare('PRAGMA table_info(platform_settings)').all<{ name: string }>();
