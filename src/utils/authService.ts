...
export class AuthService {
      ...
      if (res.success && res.data?.user) {
        ...
        const session: AuthSession = {
          ...
          user: {
            ...
            gstin: payload.gstin || '',
            planName: organization.planName || 'All-in-One Growth Plan',
      ...
      if (res.success && res.data?.user) {
        ...
        const session: AuthSession = {
          ...
          user: {
            ...
            gstin: organization?.register_number || organization?.registerNumber || '',
  ...
  static async googleAuthAsync(credential: string): Promise<{
    ...
    try {
      ...
      if (res.success) {
        if (res.data?.needsProfileCompletion && res.data?.googleProfile) {
          return {
            success: true,
            needsProfileCompletion: true,
            googleProfile: res.data.googleProfile,
          };
        }
  ...
  static async completeGoogleSignupAsync(payload: {
    credential: string;
    companyName: string;
    ownerName?: string;
    mobile: string;
    ...
    try {
      const res = await ApiService.completeGoogleSignup(payload);
      if (res.success && res.data?.user) {
        const { user, organization } = res.data;
        const expiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString();
      ...
      return {
        success: false,
        error: res.error || 'Failed to complete Google signup on server',
      };
    } catch (err: any) {
      return {
        success: false,
        error: err?.message || 'Connection error while completing Google signup',
      };
    }
  }
      ...
      if (res.success && res.data?.user) {
        ...
        const session: AuthSession = {
          ...
          user: {
            ...
            gstin: organization?.register_number || organization?.registerNumber || currentSession?.user?.gstin || '',
