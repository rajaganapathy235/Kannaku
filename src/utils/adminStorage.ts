...
export class SaaSAdminDB {
  ...
  static saveCoupon(coupon: SaaSCoupon): void {
    ...
    if (idx >= 0) {
      list[idx] = coupon;
      if (this.isSuperAdmin()) {
        ApiService.updateAdminCoupon(coupon.id, coupon).catch((err) =>
          console.error('[SaaSAdminDB] Failed to update coupon in DB:', err)
        );
      }
    } else {
      list.unshift(coupon);
  ...
  static async getPlatformSettingsAsync(): Promise<PlatformSettings> {
    if (!this.isSuperAdmin()) return this.getPlatformSettings();
    const res = await ApiService.getAdminPlatformSettings();
    if (res.success && res.data) {
      this.savePlatformSettings(res.data);
      return res.data;
    }
  ...
  static savePlatformSettings(settings: PlatformSettings): void {
    ...
    if (this.isSuperAdmin()) {
      ApiService.updateAdminPlatformSettings(settings).catch((err) =>
        console.error('[SaaSAdminDB] Failed to save platform settings in DB:', err)
      );
    }
  }
  ...
  static async getPaymentGatewaysConfigAsync(): Promise<SaaSGatewayManagerConfig> {
    if (!this.isSuperAdmin()) return this.getPaymentGatewaysConfig();
    const res = await ApiService.getAdminPaymentGateways();
    if (res.success && res.data) {
      this.savePaymentGatewaysConfig(res.data);
      return res.data;
    }
    return this.getPaymentGatewaysConfig();
  }
  ...
  static setActivePaymentGateway(provider: string): void {
    ...
    if (this.isSuperAdmin()) {
      ApiService.setActiveAdminPaymentGateway(provider).catch((err) =>
        console.error('[SaaSAdminDB] Failed to set active payment gateway in DB:', err)
      );
    }
  }
  ...
  static updateGatewayConfig(provider: string, updates: any): void {
    ...
    if (config.gateways[provider as keyof typeof config.gateways]) {
      ...
      this.savePaymentGatewaysConfig(config);
      if (this.isSuperAdmin()) {
        ApiService.updateAdminPaymentGateway(provider, updates).catch((err) =>
          console.error('[SaaSAdminDB] Failed to update gateway config in DB:', err)
        );
      }
    }
  }
